...
public class ServerBrowserScreen extends Screen {
    ...
    protected void init() {
        ...
        if (list.getEntries().size() >= 20) {
            Button nextButton = this.addButton(new Button(this.width - 110, 12, 60, 20, new TranslatableComponent("Next: " + (page + 1)), button -> {
                if (list.getEntries().size() >= 20) {
                    page++;
                    queryServers();
                    list.queryServers();
            ...
            this.addButton(new Button(this.width - 140, 12, 30, 20, new TranslatableComponent("Prev"), button -> {
                if (page <= 1) {
                    page = 1;
                } else {
                    page--;
        ...
        this.addButton(new Button(this.width / 2 - 50, 3, 100, 20, new TextComponent("Register Server"), button -> {
        ...
        this.joinButton = this.addButton(new Button(this.width / 2 - 154, this.height - 52, 100, 20, new TranslatableComponent("selectServer.select"), (button) -> {
            this.joinSelectedServer();
        }));
        this.addButton(new Button(this.width / 2 - 50, this.height - 52, 100, 20, new TranslatableComponent("Filter Servers"), (button) -> {
            this.minecraft.setScreen(new FilterServerScreen(this));
        }));
        this.addButton(new Button(this.width / 2 + 54, this.height - 52, 100, 20, new TranslatableComponent("History (WIP)"), (button) -> {
            // todo; store 20 servers last joined from this screen.
        }));
        this.favoriteButton = this.addButton(new Button(this.width / 2 - 154, this.height - 28, 70, 20, new TranslatableComponent("Favorite"), (button) -> {
        ...
        this.addButton(new Button(this.width / 2 - 80, this.height - 28, 156, 20, new TranslatableComponent("selectServer.refresh"), (button) -> {
    ...
    private void join(ServerData server) {
        this.minecraft.setScreen(new ConnectScreen(this, this.minecraft, server));
    }
